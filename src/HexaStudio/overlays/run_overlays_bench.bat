@echo off
REM ==============================================================================
REM Launch the standalone overlays bench (module-owned SettingsPanel + FakeOverlaysController).
...
set "BENCH=%~dp0..\..\..\build\bin\overlays_bench.exe"

if not exist "%BENCH%" (
    echo [run_overlays_bench] Executable not found:
    echo     %BENCH%
    echo Build it first from the repo root:
    echo     cmake --build build --target overlays_bench
    exit /b 1
)
...
echo [run_overlays_bench] Launching %BENCH% %*
"%BENCH%" %*
set "RC=%ERRORLEVEL%"
endlocal & exit /b %RC%
