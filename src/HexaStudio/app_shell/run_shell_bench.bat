@echo off
REM ==============================================================================
REM Launch the standalone assembled new-product HMI (ShellWindow + all modules + FakeBackend).
...
set "BENCH=%~dp0..\..\..\build\bin\app_shell_bench.exe"

if not exist "%BENCH%" (
    echo [run_shell_bench] Executable not found:
    echo     %BENCH%
    echo Build it first from the repo root:
    echo     cmake --build build --target app_shell_bench
    exit /b 1
)
...
echo [run_shell_bench] Launching %BENCH% %*
"%BENCH%" %*
set "RC=%ERRORLEVEL%"
endlocal & exit /b %RC%
