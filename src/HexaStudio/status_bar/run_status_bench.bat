@echo off
REM ==============================================================================
REM Launch the standalone status-bar bench (module-owned StatusBarPanel + FakeTopController).
...
set "BENCH=%~dp0..\..\..\build\bin\status_bar_bench.exe"

if not exist "%BENCH%" (
    echo [run_status_bench] Executable not found:
    echo     %BENCH%
    echo Build it first from the repo root:
    echo     cmake --build build --target status_bar_bench
    exit /b 1
)
...
echo [run_status_bench] Launching %BENCH% %*
"%BENCH%" %*
set "RC=%ERRORLEVEL%"
endlocal & exit /b %RC%
